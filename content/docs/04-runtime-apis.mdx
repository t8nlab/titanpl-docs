...
<Accordions>
<Accordion title="t.log() — Structured logging">
`t.log()` is TitanPl’s built-in logging API.  
...
<Accordion title="t.fetch() — Outbound HTTP requests">
`t.fetch()` allows actions to make outbound HTTP requests.
It follows a familiar `fetch`-style API while executing through TitanPl’s Rust networking layer.

> **Note:** `t.fetch()` must be wrapped in `drift()`. Calling it directly significantly impacts performance and will throw a runtime error.

```js
const res = drift(t.fetch("https://api.example.com/users"))
```
...
export default function example(req) {
  const res = drift(t.fetch("https://api.example.com/data"))
  return res
}
```
...
```js title="app/actions/login.js"
drift(t.fetch("https://api.example.com/login", {
  method: "POST",
  headers: {
...
```js
const res = drift(t.fetch("https://api.example.com/user/1"))
...
<Accordion title="t.jwt — JSON Web Tokens">
...
<Callout title="Production Security" type="warn">
  Always use **`process.env.JWT_SECRET`** (or a similar environment variable) for your signing secret in production. Never hardcode secrets in your source code.
</Callout>
...
<Accordion title="t.password — Password hashing">
...
<Accordion title="t.db — TitanPl Database API">
TitanPl includes a built-in, high-performance PostgreSQL client available directly in your runtime via the global `t` object. This allows you to interact with your data without installing any external Node.js dependencies.
...
2. **Connect and Query** in your action:
   ...
   export default function(req) {
       ...
       // 2. Query (using drift for non-blocking execution)
       const users = drift(db.query("SELECT * FROM users WHERE active = $1"));
...
### 📚 API Reference: `t.db.connect(connectionString)`
...
- **Arguments**:
  ...
  - `options` *(object, optional)*: Connection pool settings.
    - `max` *(number)*: Maximum number of connections in the pool (e.g., `{ max: 10 }`).
- **Returns**: A `DbConnection` object.

```js
const db = t.db.connect(process.env.DATABASE_URL, { max: 10 });
```
...
│   └── actions/
├── db/
│   └── get_active_users.sql   <-- Your SQL file
└── .env
```
...
- **Synchronous Execution**: Database queries in Titan are currently executed synchronously in the worker. **You MUST wrap `db.query()` in `drift()`**. Failure to do so will throw a runtime error.
...
<Accordion title="t.read() — Native File Reader">
...
<Callout title="Node.js Compatibility" type="info">
  If you need to use standard Node.js APIs (like `fs`, `path`, or `process`) or existing npm libraries, you can enable them using the [**@titanpl/node** compatibility layer](./15-node-compatibility).
</Callout>
...
### 🚀 Stay Up to Date
To ensure full support for the latest features and security patches, please [**update your Titan CLI and project**](./05-updates) to the latest version.
