...
Environment variables are loaded by the native runtime and made available inside **actions** via `t.env`
...
Environment variables are accessed using `t.env`

```js
const secret = t.env.JWT_SECRET
```
