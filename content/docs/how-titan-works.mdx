...
### Phase 2: Worker Execution (V8 Isolate)
...
2. **Enters V8 isolate** (isolated JavaScript environment)
3. **Calls the action function** (pre-compiled bytecode)
4. **Executes synchronously** (blocking until completion)
...
export const getUser = defineAction((req) => {
  ...
  // Blocking call to database (synchronous from JS perspective)
  const user = t.db.query(`SELECT * FROM users WHERE id = ?`, [userId]);
...
- Async I/O for network and database
- Efficient tokio runtime
- But JavaScript sees only **synchronous blocking calls**
...
└──────┬──────┘
       ...
       ▼
┌─────────────┐
│  esbuild    │ ← Bundle each action
└──────┬──────┘
       │
       ▼
┌─────────────┐
│ Rust Codegen│ ← Generate routing table
└──────┬──────┘
       │
       ▼
┌─────────────┐
│cargo build  │ ← Compile to native binary
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────┐
