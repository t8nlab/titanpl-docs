...
If you are migrating an existing Node.js project or using libraries that depend on Node.js built-ins, we recommend using the [`@titanpl/node`](./15-node-compatibility) compatibility layer. It provides shims for `fs`, `path`, `crypto`, and more, mapping them directly to the native implementations found in `@titanpl/core`.
